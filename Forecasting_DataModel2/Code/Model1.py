...
from Bias import getCenter , getCenter_
...
def LSTM(OutputFile  , X_, Y_,Trainseq_length, TestData , Testseq_length ,
	learning_rate ,n_neurons, n_layers , alpha,n_epochs,  trainKeepProb = 1.0 ,isBaseline =None , Bias1=None , Bias2=None , RID=None  , DS = 1 ,  testing=True,KmeanModel=None):
	print("Model1")
			...
			if DS ==1:
				...
				# for i in range(2):
					...
					for j in range (129-Testseq_length[i]):
						...
						X_batch_Baseline = TestData_Baseline[i].reshape(1, n_steps, n_inputs)
						y_pred_Baseline = sess.run(outputs_Last, feed_dict={X: X_batch_Baseline , seq_length:seq ,keep_prob:1.0})
						
						DefaultValues_B1 = Bias1
						DefaultValues_B2 = getCenter_(X_batch_B2[:,:seq[0],:],KmeanModel)

						TestData_b1[i , seq[0],0] =  y_pred_B1.flatten()[0]
						TestData_b2[i , seq[0],0] =  y_pred_B2.flatten()[0]
						TestData_Baseline[i , seq[0],0] =  y_pred_Baseline.flatten()[0]
						beta= getBeta(startTimeStep ,seq[0] ,  n_steps , DS=2)
						# print(beta)
						...
						for ind in range(1,n_inputs):
							if(isBaseline[ind]==1):
								...
								TestData_Baseline[i , seq[0],0] = TestData_Baseline[i , seq[0]-1,ind]
							else:
								...
								TestData_Baseline[i ,seq[0],ind] = LastValue[ind]
						
						print("compare" ,  TestData_b1[i ,seq[0],:] ,TestData_b2[i ,seq[0],:]  , TestData_Baseline[i ,seq[0],:] )
						seq[0]+=1
				...
				Output_Baseline = np.zeros((TestData_Baseline.shape[0] , TestData_Baseline.shape[1]*TestData_Baseline.shape[2]))
				colindex=0
				...
				for i in range(TestData_b1.shape[0]):
					for j in range (TestData_b1.shape[1]):
						...
						Output_Baseline[i,colindex:colindex+ TestData_Baseline.shape[2]] = TestData_Baseline[i,j,:]
				...
				df_Baseline = DataFrame(Output_Baseline)
				df_Baseline.to_csv(Loc+OutputFile+'_BL.csv',index=False)
