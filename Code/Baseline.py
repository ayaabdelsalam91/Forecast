...
def LSTM(OutputFile  , X_, Y_,Trainseq_length, TestData , Testseq_length ,
	learning_rate ,n_neurons, n_layers , alpha,n_epochs,  trainKeepProb = 1.0 , RID=None  , DS = 1 ,  testing=True):
				...
				# for i in range(1):
					...
					for j in range (129-Testseq_length[i]):
						...
						for ind in range(1,n_inputs):
							TestData[i , seq[0],ind] = TestData[i ,seq[0]-1,ind]
						seq[0]+=1
				Output = np.zeros((TestData.shape[0] , TestData.shape[1]*TestData.shape[2]))
				colindex=0
				for i in range(TestData.shape[0]):
					for j in range (TestData.shape[1]):
						# print(i,j,TestData[i,j,:])
						Output[i,colindex:colindex+ TestData.shape[2]] = TestData[i,j,:]
						colindex+=TestData.shape[2]
					colindex=0
				df = DataFrame(Output)
