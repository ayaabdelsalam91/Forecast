...
def getMAEForElectricty(NN_Output , Actual_Values,hasIndex=False , Denormalize=False , DenormalizeFile =None , FeatureIndex=3 ):
	...
	for i in range(newData.shape[0]):
		...
		ax.plot(Month, newData[i,:] , 'grey' ,  alpha=0.5)
	...
	plt.xlabel('Month')
	plt.ylabel('MAE')
	plt.title('Mean absolute error of Megawatthours for states across time')
...
Month ,Avg = getMAEForElectricty('BL_Avg_500' ,'TestingAnsDS')
...
fig, ax = plt.subplots()
ax.plot(Month, Baseline,'-gs',label='Unbiased LSTM')
# ax.plot(Month, Avg,'-p',label='LSTM with average bias')
ax.plot(Month, b2 ,'-r*',label='LSTM with cluster bias at K=2')
ax.plot(Month, b3,'-bo',label='LSTM with cluster bias at K=3')
ax.plot(Month,b4 ,'-m^',label= 'LSTM with cluster bias at K=4')
...
plt.xlabel('Month')
plt.ylabel('MAE')
plt.title('The effect of changing the number of cluster on model preformance ')
plt.savefig(Loc_Graph+"M1D2_Graph2"+'.png')
plt.show()
